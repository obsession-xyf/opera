	...
	<form id="form1" name="form1" action="" method="post">
		...
		<center>
			<table class="mobile" style="width: 95%;">
				...
				<tr height="25">
					<td class="outDetail" style="width: 30%">文件： <label
						style="font-weight: bold; color: red"> </label></td>
					<td class="outDetail2"><input id="f_fileFile0" name="cmfile"
						onchange="triggerUploadFile0(this);" title="选择附件" type="file">
						<input class="text" type="hidden" name="fileUrl" id="bbbbbFile0"
						value="${map.fileUrl }"> <c:if
							test="${map.fileUrl!=null }">
							<a href="${ctx }/${map.fileUrl}" id="aaaaaFile0">附件</a>
						</c:if></td>
				</tr>
				<tr height="25">
				...
				<tr height="25">
					<td class="outDetail" style="width: 30%">日期： <label
						style="font-weight: bold; color: red"> </label></td>
					<td class="outDetail2"><input name="showDate" id="showDate"
						value="${map.showDate }" type="text" onClick="WdatePicker()" /></td>
				</tr>
				<c:if test="${1==1 }"></c:if>
...
<script type="text/javascript">
	...
	function triggerUploadFile0(src) {
		$
				.ajaxFileUpload({
					url : '${ctx}/file/upload.json',
					secureuri : false,
					fileElementId : 'f_fileFile0',
					dataType : 'json',
					data : {
						fileloc : 'upload/',
						dir : 'temp'
					...
					success : function(data, status) {
						$("#bbbbbFile0").val(data.data.filepath);
						$("#aaaaaFile0").remove();
						$("#bbbbbFile0")
								.after(
										"<a id='aaaaaFile0' href='${ctx}/"+data.data.filepath+"' >文件</a>");
					},
					error : function(data, status, e) {
						alert('文件上传失败');
					}

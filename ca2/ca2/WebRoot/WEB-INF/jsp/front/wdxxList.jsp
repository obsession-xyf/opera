			...
			<div class="span8 main-listing">
				...
				<article class=" type-post format-image hentry clearfix">
					<header class="clearfix">
						<h3 class="post-title">
							<a href="wdxxShow.html?id=${lists.id }&bkId=${param.bkId}">${lists.title }</a>
						</h3>
						<div class="post-meta clearfix">
							<span class="date">${lists.insertDate }</span> <span class="category">
							</span>
							<span>状态：${lists.status }</span>
							<c:if test="${param.dpc!=1 }">
							<a class="btn btn-danger" href="#" onclick="deleteOne('${lists.id}');" role="button">删除</a>
							<a class="btn btn-default" href="wdxxEdit.html?id=${lists.id }&bkId=${param.bkId}" role="button">修改</a>
							</c:if>
						</div>
						<!-- end of post meta -->
					</header>
